...
main = (solve <$> getContents) >>= print where
    solve input = let
        ...
        directions = take 10 $ cycle $ dsRot
        coords' = foldl' move coords directions
        range ns = maximum ns - minimum ns + 1
        in range (map fst coords') * range (map snd coords') - length coords'
